...
@pytest.fixture()
def ddb_stream_invalid_event() -> Dict[str, Any]:
    return {
        "Records": [
            {
                "eventID": "2",
...
def test_lambda_handler_with_invalid_event(ddb_stream_invalid_event: Dict[str, Any],
    ...
    # Call the lambda handler with the invalid DynamoDB streams event
    response = app.lambda_handler(ddb_stream_invalid_event, None)
    data = json.loads(response['body'])

    assert response['statusCode'] == 500
    assert 'message' in response['body']
    assert data['message'] == "Invalid DynamoDB streams event passed (INSERT)"
    assert count_objects_in_s3_bucket(bucket) == initial_object_count
